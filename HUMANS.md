...
3. **Run the setup script** for your agent from `.ai_ops/setup/`:
   - Claude Code: `bash .ai_ops/setup/setup_claude.sh` (or `.bat` on Windows)
...
- `.agents/` = Codex/GitHub Copilot installed skills (workspace-root runtime artifact;
...
Install scope options (passed via `--workspace`, `--repo`, or `--user` flags):

- `--workspace` (recommended): installs to workspace root
  (`<workspace_root>/.agents/skills/`, `<workspace_root>/.claude/skills/`);
  workflow pointer resolves to `ai_ops/.ai_ops/workflows/`
- `--repo`: installs inside the ai_ops repo tree (optional; not required for
  export drift checks)
- `--user`: installs to user home directory
...
- GitHub Copilot (chat/agent-style usage supported; wrapper install model may differ from skills-based tools)
