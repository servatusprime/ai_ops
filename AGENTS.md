...
| `plugins/ai-ops-governance/{commands,skills}/**` | Level 1 | Generated from `.ai_ops/workflows/**` by `00_Admin/scripts/generate_workflow_exports.py`. Never hand-edit; regenerate from source. The gate is the source workflow's own Level 4. |
| `plugins/ai-ops-governance/agents/**` | Level 1 | Generated by `00_Admin/scripts/regenerate_profiles.py` via `/profiles`. Never hand-edit; regenerate from profile source data. |
...
| Reviewer | After meaningful execution phases | Judge correctness, governance fit, adequacy of decisions | 2 | plan | 20 |
...
When a task carries a delegation intent marker `[Agent: <Lane> | <tier>]`
(see `guide_workbooks.md`'s "Delegation Intent Markers (conditional)"
section) and the current surface
exposes the corresponding `ai-ops-<lane>` custom subagent (e.g.
`ai-ops-reviewer` for the `Reviewer` lane), the spawn **should** use that
subagent type rather than a generic one -- it already carries the correct
`model`, preloaded `skills`, and `tools`/`permissionMode` envelope for that
lane, so the orchestrator does not have to reconstruct it by hand. If the
surface does not expose it (custom subagent discovery is environment- and
launch-root-dependent -- see `guide_workbooks.md`'s subagent-profile
mapping), apply that profile's `tools`, `permissionMode`, and `model`
manually and record the substitution in the run's execution notes.

This is a recommended default with mandatory disclosure, not a stop-gate:
no script can observe what subagent type a harness actually spawned, so a
hard gate on the spawn act itself is unenforceable. The disclosure *is*
reviewable -- a `/crosscheck` pass can check whether execution notes record
the profile used or the substitution made.

Choose the lowest reasoning tier that can reliably complete the delegated
task. Use the lane profile default when the task shape matches it; override
the tier only when complexity or risk requires it, and record the selected
tier or model in execution notes. Use low for deterministic extraction or
lint, medium for scoped implementation, high for architecture or elevated
review, and maximum only for policy or L4 work. A delegation marker should
name the canonical lane and tier; when no marker is present, use the lane
profile default and disclose that choice.
