...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Render and fail when generated derivatives differ without writing files.",
    )
    parser.add_argument(
        "--tracked-only",
        action="store_true",
        help="With --check, compare only repository-tracked derivative surfaces.",
    )
    ...
    generated_at = str(profile.get("generated_at", "unknown"))
    profile_for_hash = dict(profile)
    profile_for_hash.pop("generated_at", None)
    source_inputs = {
        "generator_format_version": GENERATOR_FORMAT_VERSION,
        "profile": profile_for_hash,
        "rider_archetypes": RIDER_ARCHETYPES,
        "model_tuning": model_manifest,
    ...
    def write_generated(path: Path, content: str, tracked: bool = True) -> None:
        check_this = args.check and (tracked or not args.tracked_only)
        write_text(
            path,
            content,
    ...
    for role_name, spec in ROLE_SPECS.items():
        ...
        write_generated(target, rendered)
    ...
    write_generated(map_path, map_content)
    ...
    write_generated(lead_context_path, lead_context_content)
    ...
    write_generated(model_summary_path, model_summary_content)
    ...
    for role_name, spec in ROLE_SPECS.items():
        ...
        write_generated(native_target, native_content, tracked=False)
    ...
    if args.check:
        if stale_paths:
            print("[FAIL] Generated derivative drift detected:")
            for path in stale_paths:
                print(f"- {path.relative_to(repo_root)}")
            return 1
        print("Files match generated derivatives (check, no writes):")
    elif args.dry_run:
